...
moran <- function(m=NULL){
  ...
  m[is.na(m)]=0
  r_pop = raster(m)
  #meanPop = cellStats(r_pop,sum,na.rm=T)/ncell(r_pop)
  meanPop = cellStats(r_pop,sum)/ncell(r_pop)
  w = spatialWeights(nrow(r_pop)-1,ncol(r_pop)-1)
  #mor = ncell(r_pop) * cellStats(focal(r_pop-meanPop,w,sum,pad=TRUE,padValue=0,na.rm=T)*(r_pop - meanPop),sum,na.rm=T) / cellStats((r_pop - meanPop)*(r_pop - meanPop),sum,na.rm=T) / cellStats(focal(raster(matrix(data=rep(1,ncell(r_pop)),nrow=nrow(r_pop))),w,sum,pad=TRUE,padValue=0,na.rm=T),sum,na.rm=T)
  mor = ncell(r_pop) * cellStats(focal(r_pop-meanPop,w,sum,pad=TRUE,padValue=0)*(r_pop - meanPop),sum) / cellStats((r_pop - meanPop)*(r_pop - meanPop),sum) / cellStats(focal(raster(matrix(data=rep(1,ncell(r_pop)),nrow=nrow(r_pop))),w,sum,pad=TRUE,padValue=0),sum)
  return(mor)
}
...
averageDistance <- function(m=NULL,normalizeByDist=T){
  ...
  m[is.na(m)]=0 # better than removing the NAs
  r_pop=raster(m)
  norm = ifelse(normalizeByDist,1/sqrt(nrow(r_pop)*ncol(r_pop)/pi),1)
  #avgdist=cellStats(focal(r_pop,distanceMatrix(nrow(r_pop)-1,ncol(r_pop)-1),sum,pad=TRUE,padValue=0,na.rm=T)*r_pop,sum,na.rm=T) / (cellStats(r_pop,sum,na.rm=T)^2)*norm
  avgdist=cellStats(focal(r_pop,distanceMatrix(nrow(r_pop)-1,ncol(r_pop)-1),sum,pad=TRUE,padValue=0)*r_pop,sum) / (cellStats(r_pop,sum)^2)*norm
  return(list(averageDistance=avgdist))
}
...
entropy <- function(m=NULL){
  ...
  m[is.na(m)]=0
  r_dens=raster(m/sum(m))
  #r_dens=raster(m/sum(m,na.rm = T))
  #m=values(r_dens)*cellStats(r_dens,function(x,...){na.omit(log(x))})
  m=values(r_dens)*cellStats(r_dens,function(x,...){log(x)})
  m[is.na(m)]=0
